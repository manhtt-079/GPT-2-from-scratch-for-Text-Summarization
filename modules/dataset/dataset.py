    ...
    def __iter__(self):
        for src, tgt in zip(self.data.source, self.data.target):
            input_ids = self.tokenizer.encode(
                src+tgt, max_length=self.max_seq_length, truncation=True, padding='max_length')
            sep_idx = input_ids.index(self.tokenizer.sep_token_id)
            
            yield {
                'input_ids': torch.tensor(input_ids),
                'label': torch.tensor(input_ids), 
                'sep_idx': torch.tensor(sep_idx)
            }
...
def dataset(tokenizer,
    ...
    return DataLoader(dataset=tensors, batch_size=batch_size, num_workers=num_workers)